...
const userControllers = {
    addUser:async(req,res)=>{
        const {name, lastName, data, password, src, google, doc, passwordAdm} = req.body
        const hashedPassword = bcryptjs.hashSync(password)
        const userType = doc ? Doctor : Patient
        try{
            if (process.env.PASSWORD_ADM !== passwordAdm && userType == Doctor) throw new Error('No tienes autorizacion')
            let newUser = new userType({name, lastName, data, password:hashedPassword, src, google, doc})
