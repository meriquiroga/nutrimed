...
const doctorActions = {
    editProfileDoctor: (idDoctor,profileEdited) => {
        return async () => {
            try {
                let res = await axios.put(`http://localhost:4000/api/doctor/perfil/${idDoctor}`, {profileEdited})
                if (res.data.success){
                    return {success: true}
                }
    ...
    addReview:(user,id,text)=>{
        return async (dispatch,getState)=>{
            try{
                let res= await axios.put(`http://localhost:4000/api/doctor/${id}`,{text,action:'addReview'},{
                    headers:{
