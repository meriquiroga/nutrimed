...
import EveryReview from '../components/EveryReview'
...
const Reviews = ({reviews,dataUser,doctorId,userToken,addReview})=>{
    ...
    const addReviewHandler=()=>{
        setText('')
        if(text){
            addReview(userToken,doctorId,text)
            .then(res=>{
                if(res.success){
    ...
    const everyReview =newReviews.map(obj =><EveryReview key={obj._id} review={obj} user={dataUser} userToken={userToken} deleteReviewHandler={deleteReviewHandler}/>)
    return(
        <div className='divReview'>
            ...
            <div>
                <input placeholder='review' onChange={editTextReviewHandler} defaultValue={text}/>
